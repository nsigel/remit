...
export default async function UniversityPage() {
	const data = await api.university.dashboard();

	return (
		<main className="min-h-dvh">
			<header className="flex items-center justify-between border-border border-b px-8 py-6">
				<Link className="cursor-pointer font-serif text-2xl" href="/">
					Remit
				</Link>
										...
										>
											<div>
												...
												<div className="text-sm text-text-secondary">
													{p.confirmationMs != null &&
														`${(p.confirmationMs / 1000).toFixed(2)}s`}
													{p.txHash && (
														<>
															{" · "}
															<a
																className="cursor-pointer transition-colors hover:text-text"
																href={`${ARC.explorer}/tx/${p.txHash}`}
