...
export function DepositWorkflow({
	...
	const { actions, session } = useDemoSession();
	...
	const liveTopupEnabled =
		session?.student.walletMode === "circle_sca" &&
		session.liveCapabilities.topup;
	const shouldUseLiveArcDeposit =
		liveTopupEnabled && isArcSource && currency === "USDC";
	...
	const sendLiveArcDeposit = async () => {
		if (!session?.student.walletAddress) {
			throw new Error("Live Arc deposits require an active Remit wallet.");
		}

		const provider = window.ethereum;
		if (!provider) {
			throw new Error("Install a browser wallet to send a live Arc deposit.");
		}
		...
		try {
			...
			const result = await liveArcKit.send({
				from: { adapter, chain: "Arc_Testnet" },
				to: session.student.walletAddress,
				amount: formatTokenAmountForTransfer(
					amount,
					selectedCurrency?.decimals ?? 6,
				),
				token: "USDC",
			});
			...
			if (!result.txHash) {
				throw new Error(
					"The live Arc deposit did not return a transaction hash.",
				);
			}
			...
			return {
				explorerUrl:
					("explorerUrl" in result && typeof result.explorerUrl === "string"
						? result.explorerUrl
						: null) ?? `${ARC.explorer}/tx/${result.txHash}`,
			...
			throw new Error(
				error instanceof Error
					? error.message
					: "The live Arc deposit could not be sent.",
			);
		}
	};
	...
	const handleStartDeposit = async () => {
		...
		try {
			if (shouldUseLiveArcDeposit) {
				const liveResult = await sendLiveArcDeposit();

				setActiveProcessStep(processingSteps.length - 1);
				await delay(350);

				const result = actions.recordLiveTopup({
					amount: parsedAmount,
					txHash: liveResult.txHash,
					confirmationMs: liveResult.confirmationMs,
				});
				...
				if (
					...
					setCompletedTx({
						...
						explorerUrl: liveResult.explorerUrl,
					});
				}
			} else {
				await delay(
								...
								{completedTx?.txHash ? (
									<a
										...
										href={
											...
											`${ARC.explorer}/tx/${completedTx.txHash}`
										}
...
function formatTokenAmountForTransfer(value: string, decimals: number) {
	const [rawIntegerPart = "0", rawFractionalPart = ""] = value.split(".");
	const integerPart = rawIntegerPart === "" ? "0" : rawIntegerPart;
	const fractionalPart = `${rawFractionalPart}${"0".repeat(decimals)}`.slice(
		0,
		decimals,
	);
